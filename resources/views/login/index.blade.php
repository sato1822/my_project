...
<body class="login">
  <main id="login">
    ...
    <form ref="registerForm" method="post" action="/login/register">
      ...
      <div>
        PW : <input type="password" name="password" v-model="password">
      </div>
      <div>
        <button type="submit" v-on:click = "registerSubmit">送信</button>
      </div>
    </form>
    ...
    <form ref="loginForm" method="post" action="/login/sign_in">
      ...
      <div>
        <button type="submit" v-on:click="loginSubmit">送信</button>
      </div>
    </form>
    <?php } ?>
  </main>
